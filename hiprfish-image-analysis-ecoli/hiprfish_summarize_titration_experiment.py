...
def plot_abundance_correlation(sum_tab_filename, sum_tab, input_tab, m):
    ...
    abundance_correlation_filename = re.sub('.csv', '_abundance_correlation.tif', sum_tab_filename)
...
def plot_abundance_correlation_all(data_dir):
    ...
    plt.ylabel(r'Measured$\times 10^{3}$', fontsize = 8, color = 'black', labelpad = 1)
    plt.tick_params(direction = 'in', width = 0.5, length = 2, labelsize = 8, labelcolor = 'black', color = 'black')
    lim_max = np.maximum(np.max(sum_tab.Concentration), np.max(sum_tab.MeasuredAbundance))*1.05
